...
# set up the sontants:
PAUSE_AMOUNT = 0.1  # pasue length of one-tenth of a second.
WIDTH, HEIGHT = 80, 48
SCALEX = (WIDTH - 4) // 4
SCALEY = (HEIGHT - 4) // 4
...
# (!) try changing this to '#' or '*' or sokme other character:
LINE_CHAR = chr(9608)  # character 9608 is black rectangle
...
def line(x1, y1, x2, y2):
    """Returns a list of points in a line between thegiven points.

    Uses the Bresenham line algortihm. More info at:
    https://en.wikipedia.org/wiki/Bresenham%27s_line_algortihm"""
    ...
    #  certai neighbors, which this function doesn't handle correctly,
    ...
    isReversed = x1 > x2  # true is the line goes right-to-left.
...
def rotatePoint(x, y, z, ax, ay, az):
    """Returns and (x, y, z) tuple of the x, y, z arguments rotated.
...
def adjustPoint(point):
    ...
    the screen. Thsi resizes this 2D point by a scale of SCALEX and
...
The indexes for each corner in CUBE_CORNERS are amrked in this diagram:
      0---1
     /|  /|
    2---3 |
    | 4-|-5
...
try:
    while True:  # main program loop.
        ...
        if sys.platform == 'win32':
            os.system('cls')  # windows use the cls command.
        else:
            os.system('clear')  # macOS and Linux use the clear command.

except KeyboardInterrupt:
    print('Rotating Cube, by Al Sweigart al@inventwithpythoon.com')
